...
module.exports = {
  ...
  #top_button { border-radius: 50%; position: fixed; right: 5vw; bottom: 10vh; background-color: #111114; color: white; height: 10vw; width: 10vw; font-size: 3vw; box-shadow: 5px 5px 5px 5px rgba(0, 0, 0, 0.3); z-index: 2; border-width: 1; } .visible { display: inline-block; animation: opacity-on 1s; } .unvisible { display: none; } @keyframes opacity-on { 0% { opacity: 0; } 100% { opacity: 1; } }
  ::-moz-selection { background-color:white; color: black}::selection {background-color:white;color: black;}::-webkit-scrollbar { width: 2vw;}::-webkit-scrollbar-track { background: white; } ::-webkit-scrollbar-thumb { background: #888; }::-webkit-scrollbar-thumb:hover { background: #555; }
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <script>window.onscroll = function () { TopButton() }; function TopButton() { window.onscroll = function (ev) { if (window.scrollY > 10) { document.getElementById('top_button').className = "visible"; } else { document.getElementById('top_button').className = "unvisible"; } }; }</script>
  

            <title>漢字勉強 - ${title}</title>
        </head>
        <body>
    <button id="top_button" class="unvisible" onclick="window.scroll({top: 0,behavior: 'smooth'});;">TOP</button>
