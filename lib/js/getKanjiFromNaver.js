...
module.exports = {
    getKanji: async function (speedmode, callback_write, callback_end, callback_console) {
        ...
        callback_write('moving to kanji dictionaty..', 15);
        ...
        if (speedmode === "true") {
            total_page = 5;
            (`<p style="color:green">SPEEDMODE ACTIVATED : SCAN ONLY 5 PAGES</p>`, 20);
        }

        callback_write(`Total Page: <div style="color:green;display:inline-block">` + total_page + `</div>`, 20);

        for (var i = 1; i <= total_page; i++) {
            console.log("saving page no." + i + "...");
            callback_write(("saving page no." + i + "..."), `${20 + (i / (total_page)) * 70}`);
                    ...
                    if (key === last_kanji) {
                        if (total != 0) {
                            ...
                            if (total == 0) {
                                ...
                                console.log(responseText);
                                callback_console(`<script>$('#console').attr({'style':'box-shadow: 5px 5px 5px 5px rgba(0, 0, 0, 0.3);overflow:scroll;overflow-x: hidden;width:70vw;height:70vh;margin:auto;text-align:center;background-color: black;color: white;font-size:2vw;'})</script>`)
                                callback_console(`<script>$('#progressbar').css('background-image','repeating-linear-gradient(45deg, #ff4242, #ff4242 10px, #ff2424 10px, #ff2424 20px)');</script>`);
                                callback_end(`<p style="color:red">${responseText}</p>`);
                            }
                        }
                        break;
                        ...
                        if (sync == 0) {
                            ...
                            callback_write(`{${key} : ${kanjiDict[key]}} 을(를) ${section_max + 1}번째 섹션에 ${datetime}시에 저장 완료했습니다.`, 100);
                            total++;
                        }
                    }
                }
